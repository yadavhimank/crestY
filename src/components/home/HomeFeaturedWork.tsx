    ...
    >
      ...
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 18, borderTop: "1px solid var(--m-line)", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          <div>
            <div className="m-display" style={{ fontSize: 28, letterSpacing: "-0.025em" }}>{metric[0]}</div>
            <div className="m-mono" style={{ fontSize: 10, color: "var(--m-ink-4)", letterSpacing: "0.06em", textTransform: "uppercase", marginTop: 2 }}>{metric[1]}</div>
          </div>
  ...
  if (kind === "compass") {
    return (
      <svg viewBox="0 0 320 220" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        ...
        {([[30,60],[110,60],[110,110],[190,110],[190,60],[270,60],[30,130],[90,130],[90,180],[230,180],[230,130],[290,130]] as [number,number][]).map(([x,y],i) => (
    ...
    return (
      <svg viewBox="0 0 320 220" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        <g stroke="var(--m-accent)" strokeWidth="1" fill="none" opacity="0.9">
          {[60, 100, 140].map(y => <line key={y} x1="40" y1={y} x2="280" y2={y} />)}
          {[80, 140, 200, 240].map(x => <line key={x} x1={x} y1="40" x2={x} y2="180" />)}
        </g>
